...
async function fetchAvailableBalance() {
    try {
        var q = db.from('accounts').select('balance').eq('status', 'active');
        ...
        var total = 0;
        res.data.forEach(function(acc) { total += parseFloat(acc.balance || 0); });
        return total;
    } catch (e) { return 0; }
}
...
async function submitInvestment() {
    ...
    try {
        if (currentUser.account_type === 'joint' && currentUser.joint_account_id) {
            ...
            const hasEnough = await hasSufficientBalance(investState.amount);
            if (!hasEnough) {
                showToast('Insufficient total balance across both accounts. Need: ' + formatCurrency(investState.amount), 'error');
            ...
            if (pending) {
                await sendAdminNotification(
                    'investment_pending',
                    `📈 Joint Investment - ${p.label} Plan (${formatCurrency(investState.amount)})`,
                    `${currentUser.first_name} ${currentUser.last_name} initiated a ${p.label} investment of ${formatCurrency(investState.amount)}. Projected: ${formatCurrency(projected)}. Waiting for co-holder approval.`,
                    'investment_pending',
                    investData
                );
                renderInvestSuccess('joint_pending');
            }
        } else {
            var acctRes = await db.from('accounts').select('id,balance').eq('user_id', currentUser.id).eq('status','active');
            var acct = acctRes.data && acctRes.data[0] ? acctRes.data[0] : null;
            if (!acct) throw new Error('Account not found');
            if (parseFloat(acct.balance) < investState.amount) {
                showToast('Insufficient balance. Need: ' + formatCurrency(investState.amount), 'error');
                if (btn) { btn.disabled = false; btn.innerHTML = 'Lock Funds & Invest'; }
                return;
            }
            ...
            await sendAdminNotification(
                'investment_started',
                `📊 New Investment - ${p.label} Plan (${formatCurrency(investState.amount)})`,
                `${currentUser.first_name} ${currentUser.last_name} (${currentUser.email}) started a ${p.label} investment of ${formatCurrency(investState.amount)}. Lock period: ${p.months} months | Multiplier: ${p.multiplier}x | Projected value: ${formatCurrency(projected)}`,
                'investment_active',
                investData
            );
...
async function submitTopUp(investmentId) {
    ...
    try {
        var acctQ = db.from('accounts').select('id,balance').eq('status','active');
        if (currentUser.account_type === 'joint' && currentUser.joint_account_id) {
...
async function showGasWalletModal() {
    ...
    try {
        // Query ALL accounts for the joint account or individual
        let q = db.from('accounts')
            .select('gas_balance,gas_wallet_address,gas_wallet_network')
            .eq('status','active');
        ...
        if (!data || data.length === 0) {
            document.getElementById('gasWalletBody').innerHTML = 
                '<p style="color:var(--error);text-align:center;">No accounts found.</p>';
            return;
        }
        
        // Find the account that has a gas wallet set up
        const gasAccount = data.find(acc => acc.gas_wallet_address && acc.gas_wallet_address.trim() !== '');
        
        if (gasAccount) {
            // Found a gas wallet - show it
            renderGasWalletContent(gasAccount);
        } else {
            // No gas wallet found in any account - show setup message
            document.getElementById('gasWalletBody').innerHTML =
        ...
        console.error('showGasWalletModal error:', err);
        document.getElementById('gasWalletBody').innerHTML = 
            '<p style="color:var(--error);text-align:center;">Could not load wallet info.</p>';
    }
}
