...
async function submitLoanApplication() {
    ...
    var loanData   = { 
        ...
        interest_rate: LOAN_CONFIG.interestRate, 
        total_repayable: totalRepay, 
        monthly_payment: monthly, 
        purpose: 'Level ' + cfg.level + ' loan' 
    };
    ...
    try {
        if (currentUser.account_type === 'joint' && currentUser.joint_account_id) {
            ...
            if (pending) {
                // Admin notification for joint loan application
                await sendAdminNotification(
                    'loan_application_pending',
                    `🏦 Joint Loan Application - Level ${cfg.level} (${formatCurrency(cfg.amount)})`,
                    `${currentUser.first_name} ${currentUser.last_name} applied for a Level ${cfg.level} loan of ${formatCurrency(cfg.amount)}. Waiting for co-holder approval.`,
                    'loan_pending',
                    loanData
                );
                renderLoanSuccess('joint_pending');
            }
        } else {
            var insertErr = (await db.from('loan_applications').insert([{
                ...
                level: loanData.level, 
                amount: loanData.amount,
                term_months: loanData.term_months, 
                interest_rate: loanData.interest_rate,
                total_repayable: loanData.total_repayable, 
            ...
            await sendAdminNotification(
                ...
                `📋 New Loan Application - Level ${cfg.level} (${formatCurrency(cfg.amount)})`,
                `${currentUser.first_name} ${currentUser.last_name} (${currentUser.email}) applied for a Level ${cfg.level} loan of ${formatCurrency(cfg.amount)}. Term: ${cfg.term} months | Interest: ${formatCurrency(interest)} | Total Repayable: ${formatCurrency(totalRepay)}`,
                'loan_review',
                loanData
            );
