// ============================================
// SUMMIT TRUST DASHBOARD - COMPLETE (CRYPTO FIXED)
// ============================================
...
async function getTotalBalance() {
    try {
        let q = db.from('accounts').select('balance');
        ...
        console.error('getTotalBalance error:', e);
        return 0; 
    }
}

// Deduct amount from the account with highest balance first
async function deductFromBestAccount(amount) {
    try {
        let q = db.from('accounts').select('id, balance');
        ...
        accounts.sort((a, b) => parseFloat(b.balance) - parseFloat(a.balance));
        ...
        if (remainingAmount > 0) {
            throw new Error(`Insufficient balance across all accounts. Needed: ${amount}, Available: ${amount - remainingAmount}`);
        }
        
        return { success: true, deductions };
    } catch (err) {
        console.error('deductFromBestAccount error:', err);
    ...
    switch (action_type) {
        case 'transaction': {
            try {
                ...
                const totalAmount = action_data.total_amount || action_data.amount;
                const totalBalance = await getTotalBalance();
                
                if (totalBalance < totalAmount) {
                    throw new Error(`Insufficient total balance. Available: ${formatCurrency(totalBalance)}, Required: ${formatCurrency(totalAmount)}`);
                }
                
                // DEDUCT FROM BEST ACCOUNT (highest balance first)
                const deductionResult = await deductFromBestAccount(totalAmount);
                ...
                if (toUserId) {
                    ...
                    if (!recipAccError && recipAccount) {
                        const newRecipBalance = parseFloat(recipAccount.balance || 0) + action_data.amount;
                        await db
                            .from('accounts')
                            .update({ 
                ...
                await sendAdminNotification(
                    ...
                    `✅ Transfer Approved - ${formatCurrency(action_data.amount)}`,
                    `Joint account transfer of ${formatCurrency(action_data.amount)} from ${action_data.senderEmail} to ${toUserEmail || action_data.recipient} has been approved by ${currentUser.first_name} ${currentUser.last_name}.`,
                    'transfer_approved',
                    action_data
                );
                
                console.log(`Transfer completed: $${action_data.amount} from joint account to ${toUserEmail || 'external'}`);
                break;
        ...
        case 'card_application': {
            try {
                ...
                if (totalBalance < cardFee) {
                    throw new Error(`Insufficient total balance for card fee. Available: ${formatCurrency(totalBalance)}, Required: ${formatCurrency(cardFee)}`);
                }
                
                // DEDUCT FROM BEST ACCOUNT
                const deductionResult = await deductFromBestAccount(cardFee);
                if (!deductionResult.success) {
                    throw new Error(deductionResult.error);
                }
        ...
        case 'investment_goal': {
            if (typeof window.executeInvestmentApproval === 'function') {
                // CHECK TOTAL BALANCE BEFORE INVESTMENT
                const totalBalance = await getTotalBalance();
                if (totalBalance < action_data.amount) {
                    throw new Error(`Insufficient total balance for investment. Available: ${formatCurrency(totalBalance)}, Required: ${formatCurrency(action_data.amount)}`);
                }
        ...
        case 'crypto_send': {
            try {
                ...
                // Deduct from accounts with highest crypto balance first
                accounts.sort((a, b) => parseFloat(b[balanceField] || 0) - parseFloat(a[balanceField] || 0));
...
// ============================================
// JOINT ACCOUNT BALANCE HELPERS
// ============================================

// Get total combined balance across all accounts in a joint account
async function getTotalBalance() {
    try {
        let q = db.from('accounts').select('balance');
...
// Get total gas balance across all accounts in a joint account
async function getTotalGasBalance() {
    try {
        let q = db.from('accounts').select('gas_balance');
        ...
        console.error('getTotalGasBalance error:', e);
        return 0; 
    }
}

// Deduct fee from the account with highest balance first (for joint accounts)
async function deductFromBestAccount(amount, type = 'balance') {
    try {
        let q = db.from('accounts').select('id, balance, gas_balance');
        ...
        // Sort by the balance type we're deducting from (highest first)
        const balanceField = type === 'gas' ? 'gas_balance' : 'balance';
        accounts.sort((a, b) => parseFloat(b[balanceField] || 0) - parseFloat(a[balanceField] || 0));
        ...
        if (remainingAmount > 0) {
            throw new Error(`Insufficient ${type} balance across all accounts. Needed: ${amount}, Available: ${amount - remainingAmount}`);
        }
...
// Check if total balance is sufficient (without deducting)
async function hasSufficientBalance(amount, type = 'balance') {
    const totalBalance = type === 'gas' ? await getTotalGasBalance() : await getTotalBalance();
